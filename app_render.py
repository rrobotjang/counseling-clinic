"""
심리상담 클리닉 - 경량 버전 (Render.com용)

한국어 모델 없이 실행 - RL 상담사 + DB 응답만 사용
"""
...
def initialize_clinic():
    ...
    counseling_client = db
    
    client_list = "\n".join([f"• {c.name} ({c.age}세, {c.gender}) - {', '.join(c.issues)}" for c in clients.values()])
    
    return f"심리상담 클리닉이 초기화되었습니다.\n\n등록된 내담자:\n{client_list}\n\nDB: {db.get_statistics()['qa_pairs']}개 QA 로드됨"
...
def counseling_chat(user_message: str, history: list):
    global current_client_name, current_session_active
    ...
    db_response = None
    if db:
        result = db.get_best_response(user_message, category)
        if result and result['response']:
            db_response = result['response']
    
    response = db_response if db_response else rl_response
        ...
        with gr.Tabs():
            ...
            with gr.Tab("상담 대화"):
                ...
                def respond(message, history):
                    new_history, response, status = counseling_chat(message, history)
